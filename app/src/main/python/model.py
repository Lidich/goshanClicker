...
class GoshanBrain:
    ...
    def predict(self, screenshot, ms_since_last):
        screenshot = self.base64_to_numpy(screenshot)
        return self.predict_brute_force(screenshot, ms_since_last)


    def base64_to_numpy(self, base64_str) -> np.ndarray:
        # Декодируем Base64 в байты
        image_bytes = base64.b64decode(base64_str)

        # Открываем изображение через PIL и конвертируем в RGB (убираем альфа-канал)
        image = Image.open(io.BytesIO(image_bytes)).convert("RGB")

        # Конвертируем в numpy-массив (только 3 канала)
